TEST_STRING="Vestibulum at odio ac ipsum dictum fringilla ultricies non dui. Duis ultrices vestibulum dolor a dictum. Etiam at elit non purus euismod fringilla. Donec non vestibulum nunc. Fusce id sem nisl. Nunc interdum iaculis erat, non congue sapien fringilla ut. Nulla dictum dolor et eleifend ullamcorper. Donec arcu elit, porttitor ac lacus a, eleifend viverra est. Sed egestas, elit sed blandit aliquet, eros lacus maximus mauris, a sagittis tortor velit nec justo. Etiam ultricies ligula non dictum accumsan. Praesent lacinia felis sit amet vehicula venenatis. Proin hendrerit elit eu ipsum sodales, vitae pulvinar arcu feugiat. Vivamus sagittis in arcu molestie varius. Phasellus mattis placerat odio vel laoreet. Vestibulum at ligula id mi commodo commodo.
...
class LempelZiv
  ...
  def encode(input)
    ...
    # puts "Original Dictionary: #{dictionary}"

    w = ""
    result = []
    input.each_char do |char|
      wchar = w + char
      if dictionary.has_key?(wchar)
        w = wchar
      else
        result << dictionary[w]
    ...
    # puts "Final Dictionary: #{dictionary}"
    result
  end
  ...
  def decode(input)
    ...
    w = result = input.shift
    input.each do |char|
      if dictionary.has_key?(char)
        entry = dictionary[char]
      elsif char == dict_size
        entry = w + w[0,1]
      else
        raise "BAD COMPRESSED item: #{char}"
      end
      result += entry
...
File.open("compressed.marshal","w") do |file|
  compressed=lz.encode(TEST_STRING)
